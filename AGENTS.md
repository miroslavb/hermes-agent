...
Обычный Telegram turn и безвредные tools проверены в текущем аудите. Следующий шаг — отдельно согласовать исправление конфликта child wall-clock/ожидания approval и выполнить native approve/deny round-trip с единственной доставкой и продолжением сессии. Старые duplicate Home Assistant credential, MCP reconnect и ошибка checkpoint Git-store воспроизводятся до и после обновления; не объявлять все адаптеры/чекпойнты исправными. Текущий systemd TimeoutStopUSec=330 секунд уже устраняет старое startup-предупреждение о 90 секундах. Конфигурация, сервисы, профили и checkpoint store этим аудитом не менялись. [Source: /root/hermes-agent-fix-20260811/docs/UPDATE_AUDIT_2026-09-07.md; pid-error-comparison.json; live systemd/config/Git probes, 2026-09-07]
