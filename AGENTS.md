...
На 2026-09-07 production1b3708005d объединён с подготовленным обновлением в staging. В изолированном candidate runtime прошли 917 тестов в 59 файлах, TUI/web и direct-Hermes model/tool canary. Исправлена реальная ошибка upstream: абсолютный `/root/.ssh/authorized_keys` теперь распознаётся как чувствительный redirect. [Source: docs/UPDATE_RECOVERY_2026-09-06.md; activation-20260907/candidate-tests.log, 2026-09-07]

В Telegram Hermes использует собственный цикл через direct codex_responses; вложенный Codex не является условием обновления. Сохранены привязки approval к запросу, чату, сообщению и предложенным кнопкам, включая moved `gateway/run_turn_runner.py` и canonical `tools.approval_context`. [Source: User correction; final regression suite, 2026-09-07]
