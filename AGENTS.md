...
Staging verification: 288 focused tests passed; TUI and web builds succeeded.
The broad run is not green (45,373 passed / 143 failed / 380 skipped plus one
no-result file); use its pure-upstream comparison before drawing conclusions.
No production activation: the concurrent approvals owner has uncommitted,
overlapping live changes after baseline `666d5bd3b6`. Integrate that owner's
finished commit before deployment; do not overwrite or auto-commit its edits.
The incident sections below describe the pinned pre-merge snapshot, not a fresh
claim about the running gateway. Native policy now lives in turn_stop_gates /
turn_final_response, delegated input composition in conversation_loop, steering
in interrupt_control, and final-delivery identity in gateway/run_turn.
...
## Codex approvals repair — draft 2026-09-06
...
**Ремонт Codex approvals — 2026-09-06 21:50 UTC: проверенный черновик, НЕ применён.** Оператор явно поручил ремонт после включения Hermes approvals.mode=manual. Подтверждены CLI-only callback=None при зарегистрированном gateway listener, сохранение bypass-решения на время жизни Codex-сессии и принятие нового запроса устаревшей Telegram-кнопкой (FIFO без request ID). В изолированной копии подготовлен патч: штатная очередь Hermes и актуальный режим на каждый запрос; только once/deny; привязка Telegram-кнопки к request ID, чату, message ID и разрешённым вариантам. Пропавший UI, отказ, таймаут, interruption, ошибка доставки и чужие/устаревшие ответы закрываются отказом. 144/144 теста прошли, git apply --check прошёл. Очередь, протокол Codex и Telegram adapter/callback упражнялись реально внутри тестов; сеть/SDK и источник policy подменены, живое подтверждение в Telegram не проверялось. Рабочий runtime-код/конфиг/процесс не менялись: auto_review дважды отклонил применение, считая разрешение из переписки untrusted transcript. Намеренно падающий тест удалён из рабочего дерева; патч включает его окончательную форму. Патч не меняет Codex auto_review и не отменяет его отказы до появления UI-запроса. Cron Hopper и два отчёта остаются не установлены/не запланированы; повторное согласие пользователя не требуется. Следующий шаг — принятие точного патча управляющим механизмом разрешений, затем штатная активация после завершения активных ходов и реальная проверка кнопок. Доказательства: /root/.local/state/codex-approval-repair-20260906/HANDOFF.md, repair.patch, manifest.json, final-tests.txt, receipt.json. Связи [[projects/pearl-hopper]], [[projects/gbrain]], [[projects/ha-voice-control]].
