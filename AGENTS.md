...
## Operations repair — 2026-09-07, before activation

К 7 сентября 2026 checkpoint store восстановлен без изменения 213 исходных файлов; fsck и новый безопасный snapshot/restore прошли. Notion MCP восстановлен на действующем ключе, 24 инструмента и API-get-self проверены, resident gateway снова connected. Исправление учёта human approval и отключение только лишних HA-адаптеров havoice/herald проверены, но пока ожидают нового gateway-процесса. 69 файлов/1110 тестов прошли без ошибок и пропусков; full broad suite не объявляется green. [Source: /root/hermes-agent-fix-20260811/docs/OPS_REPAIR_2026-09-07.md; /root/.local/state/hermes-ops-repair-20260907/regression-final/summary.json; live recovery receipts, 2026-09-07]

Keep approvals manual/900s and the child's 600s execution budget. Exclude only owner-scoped bounded human waits; never sibling waits or arbitrary slow tools. Preserve the verified checkpoint backup, existing data and all profile secrets. The havoice voice bridge and direct HA tools must remain available; only the duplicate event adapters are disabled. Next: native after-turn graceful restart and bounded postactivation proof of the expected source tree/new PID/Telegram/main HA/Notion. Do not claim this code active from tests or a Git merge; exact live activation evidence is required. [Source: User and docs/OPS_REPAIR_2026-09-07.md, 2026-09-07]
