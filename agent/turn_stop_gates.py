...
def _completion_policy_nudge(
    agent, final_response, attempt: int, *, turn_id: str, user_message: Any
) -> Optional[str]:
    """Let a local plugin request one bounded continuation for any successful turn."""
    try:
        from agent.completion_hooks import completion_continue_message

        return completion_continue_message(
            agent,
            turn_id=turn_id,
        ...
        logger.debug("pre_turn_complete hook check failed", exc_info=True)
        return None
...
def apply_stop_gates(
    ...
    pending_verification_response_previewed: Any, turn_id: str = "", user_message: Any = "",
) -> StopGateVerdict:
    ...
    if not _verify_nudge2:
        _verify_nudge2 = _completion_policy_nudge(
            agent, final_response, _attempt, turn_id=turn_id, user_message=user_message
        )
    if _verify_nudge2:
