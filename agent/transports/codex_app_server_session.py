...
class CodexAppServerSession:
    ...
    def __init__(
        ...
        codex_home: Optional[str] = None, codex_extra_args: Optional[list[str]] = None,
        resume_thread_id: Optional[str] = None, recovery_context: Optional[str] = None,
    ...
    def ensure_started(self) -> str:
        ...
        if self._client is None:
            ...
            # Keep injected factories that predate launch overrides compatible.
        ...
        if self._resume_thread_id:
            try:
                ...
                # A pruned rollout can start fresh because run_turn() seeds the
                # replacement from the canonical Hermes transcript.
                logger.warning(
    ...
    def resumable_thread_id(self) -> Optional[str]:
        """Return the thread only after it is known to contain canonical state."""
    ...
    def run_turn(
        ...
        """Send a user message and block until turn-level completion.

        ``turn_timeout`` is an inactivity bound refreshed by each in-scope
        notification or server request. The optional post-tool timeout is off by
        default because quiet reasoning after a large tool result is not a wedge.
        """
        result = TurnResult()
        if self._start_for(result):
            # Do not clear first: a hard stop arriving during startup must be honored.
            if self._interrupt_event.is_set():
                ...
                if on_dispatch is not None:
                    try:
                        on_dispatch(copy.deepcopy(params))
                    except Exception:
    ...
    def _run_started_turn(
        ...
        post_tool_quiet_timeout: Optional[float],
    ) -> None:
        ...
        def watchdog_tripped() -> bool:
            if (
                post_tool_quiet_timeout is None
                or last_tool_completion_at is None
                or (time.monotonic() - last_tool_completion_at) <= post_tool_quiet_timeout
            ):
                return False
        ...
        self._drive_turn(
            ...
            on_note=on_note, confirm_turn_state_at_deadline=True,
        )
        with self._active_turn_lock:
            self._active_turn_id = None
    ...
    def _drive_turn(
        ...
        confirm_turn_state_at_deadline: bool = False,
    ) -> None:
        """Shared inactivity-bounded poll loop for turns and compaction.

        Every in-scope notification or server request refreshes the deadline;
        productive long turns therefore remain live. Item-level assistant text
        is never promoted to final without turn-level completion evidence.
        """
        ...
        if (
            confirm_turn_state_at_deadline
            ...
            if normalized_status == "completed":
                logger.warning(
                    ...
                    "confirmed completion, accepting the final assistant text"
                )
                turn_complete = True
                result.should_retire = True
                ...
                result.error = self._format_error_with_stderr(
                    ...
                    _format_responses_error(err_obj, status) if err_obj else status,
                )
                turn_complete = True
                result.should_retire = True
        ...
        if not turn_complete and not result.interrupted:
            ...
            if not result.error:
                result.error = self._format_error_with_stderr(
                    f"{timeout_label} timed out after {turn_timeout}s without activity"
                )
            result.should_retire = True

    def _read_turn_state(self, turn_id: Optional[str]) -> Optional[dict[str, Any]]:
        """Read one stored turn to recover a dropped completion notification."""
