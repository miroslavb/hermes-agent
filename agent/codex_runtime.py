...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    ...
    cache_read_tokens = min(
        _coerce_usage_int(usage.get("cachedInputTokens")),
        input_tokens_total,
    )
    # App-server reports cachedInputTokens as a subset of inputTokens. Canonical
    # usage stores disjoint buckets, so subtract it to avoid double-counting.
    canonical_usage = CanonicalUsage(
        input_tokens=input_tokens_total - cache_read_tokens,
...
def make_codex_app_server_event_bridge(agent) -> Callable[[dict], None]:
    ...
    def _fire_agent_message_completed(item: dict) -> None:
        ...
        if not isinstance(text, str) or not text.strip():
            return
        # The final_answer item is returned through the normal terminal path.
        # Sending it through the interim lane gives one response two owners.
    ...
    def on_event(note: dict) -> None:
        ...
        # The transport filters foreign events before this callback. Stamp real
        # provider progress even when all visible commentary callbacks are off.
        if method:
            agent_cb(
                "_touch_activity",
                "Codex activity projection failed",
                args=(f"codex app-server: {method}",),
            )
...
def _ensure_codex_session(agent, messages: List[Dict[str, Any]]) -> None:
    """Lazily spawn one resumable Codex session per AIAgent."""
...
def _finish_codex_turn(
    ...
    *,
    ...
    """Post-turn bookkeeping mirroring the chat_completions loop."""
    agent._iters_since_skill = getattr(agent, "_iters_since_skill", 0) + sum(
        observed.tool_iterations for observed in turns
    )
    ...
    for observed in turns:
        _record_codex_app_server_compaction(agent, observed)
        usage_result = _record_codex_app_server_usage(
            agent,
            observed,
            messages=messages if observed is turn else None,
        )
    ...
    if turn.final_text and not turn.interrupted and (
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    """Hand a turn to app-server while preserving one durable Codex thread."""
    ...
    def observe_dispatch(params: dict) -> None:
        invoke_hook(
            "pre_api_request",
            ...
            request_scope="runtime_turn",
            runtime="codex_app_server",
            request_messages=[
                {"role": "user", "content": params["input"][0]["text"]}
            ],
            system_prompt="",
            api_call_count=len(runtime_turns) + 1,
        )
    ...
    try:
        ...
        return _turn_result(
            _consume_user_interrupt(agent), messages, api_calls=len(runtime_turns),
