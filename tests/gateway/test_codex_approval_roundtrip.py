...
@pytest.mark.asyncio
@pytest.mark.parametrize("request_source", ["hermes", "codex-protocol"])
@pytest.mark.parametrize("case", ["once", "deny", "stale", "restart", "wrong_chat", "wrong_message", "unoffered_choice", "unauthorized"])
async def test_telegram_response_is_bound_to_request(monkeypatch, case, request_source):
    stale = case in {"stale", "restart"}
    monkeypatch.setenv("HERMES_GATEWAY_SESSION", "1")
    monkeypatch.setattr(approval_context, "_get_approval_config", lambda: {"mode": "manual"})
    monkeypatch.setattr(approval_context, "_get_approval_timeout", lambda: 0.15 if stale else 3)
    ...
    async def send(data):
        ...
        assert buttons[0].text == "✅ Allow Once" and buttons[-1].text == "❌ Deny"
    ...
    session = (CodexAppServerSession(cwd="/tmp", approval_callback=approval.request_runtime_approval)
               if request_source == "codex-protocol" else SimpleNamespace())
    session._client = SimpleNamespace(respond=lambda rid, data: replies.append((rid, data["decision"])))
    def request(rid):
        if request_source == "hermes":
            result = approval.request_tool_approval("terminal", "integration approval test", rule_key="test-direct-binding")
            replies.append((rid, "accept" if result.get("approved") else "decline"))
            return
    ...
    async def click(card, attempt="once"):
        ...
        if attempt in {"deny", "unoffered_choice"}:
            data = data.replace("ea:once:", "ea:deny:" if attempt == "deny" else "ea:not_offered:")
