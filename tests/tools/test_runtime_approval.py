...
def ui(monkeypatch):
    ...
    monkeypatch.setattr(approval, "_get_approval_timeout", lambda: 0.08)
...
def test_concurrent_sessions_cannot_consume_other_answer(ui, monkeypatch):
    ...
    def request(key):
        ...
        finally: approval.reset_current_session_key(token)
    try:
