...
Исторический ранний post-start срез 02:07 UTC ещё не включал обычный пользовательский Telegram turn и native approve/deny round-trip; новых ERROR/CRITICAL тогда не выявили. Поздняя проверка, обнаруженные старые ошибки и актуальные следующие шаги приведены в дневном аудите ниже. [Source: activation-20260907/postactivation-verification.json; docs/UPDATE_AUDIT_2026-09-07.md, 2026-09-07]

### Daytime read-only audit — 2026-09-07
...
Обычный Telegram turn и tools текущего аудита уже работают. Следующие отдельные работы: согласовать взаимодействие child deadline и approval wait, затем проверить живой approve/deny; сохранить и восстановить неисправный checkpoint store без очистки данных. До и после активации воспроизводятся те же ошибки duplicate HA credentials, MCP reconnect и `not a git repository` для checkpoints; работа всех адаптеров не принята. Startup timeout warning о 90 секундах исторический: loaded systemd уже показывает330. Никаких runtime/config/profile изменений этот аудит не вносил. [Source: [полный отчёт](UPDATE_AUDIT_2026-09-07.md); fresh PID-scoped journal comparison and read-only probes, 2026-09-07]
...
`f30be09fd15ad138c8fc47184bbbd26ec3a5324f` is now reconciled with the verified
candidate `e6761a8045d95852ee96528e2f593c19ca553d25`. Executable code is unchanged
from that candidate; all documentation-only history is retained below and in
Git. This authorization permits a new controlled attempt, not bypassing the
idle, identity or clean-tree guards. [Source: User and Git comparison, 2026-09-07]

A fresh preflight before this documentation merge passed with gateway idle;
production has NOT yet been activated. The installer must re-pin OLD to the
latest production documentation commit and NEW to this reconciled merge, then
repeat preflight and validate its guards before launch. It preserves the managed
Python3.11.15/SQLite3.53.1 candidate, existing venvs, profile data and native
Hermes `codex_responses` / manual approval policy. Default compression remains
0.75 as expressly requested by the owner. [Source: current preflight, candidate
receipts and User, 2026-09-07]

Acceptance still requires new process and exact loaded commit, correct editable
source/interpreter, connected Telegram and a real post-update native approval
round-trip. The saved candidate suite is 59 files/917 passed/0 failed; full-suite
green is NOT claimed. The actual absolute-home sensitivity defect was fixed in
e6761a8045, not waived as an upstream failure. [Source: committed-tests.log,
direct-runtime-receipt.json and candidate Git diff, 2026-09-07]

## Historical blocked activation — verified 2026-09-07 01:39 UTC
